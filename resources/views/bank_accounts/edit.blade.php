        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('bank-accounts.update', $bankAccount) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="branch_name" class="block mt-1 w-full" type="text" name="branch_name" :value="old('branch_name', $bankAccount->branch_name)" required />
                        </div>

                        {{-- Nama Pemilik Rekening --}}
                        <div class="mt-4">
                            <x-input-label for="account_holder_name" :value="__('Nama Pemilik Rekening')" />
                            <x-text-input id="account_holder_name" class="block mt-1 w-full" type="text" name="account_holder_name" :value="old('account_holder_name', $bankAccount->account_holder_name)" required />
                        </div>
                        ...
                        {{-- Logo Bank --}}
                        <div class="mt-4">
                            <x-input-label for="bank_logo" :value="__('Ganti Logo Bank (Opsional)')" />
                            <x-text-input id="bank_logo" class="block mt-1 w-full border-gray-600 p-2" type="file" name="bank_logo" />
                            @if($bankAccount->bank_logo)
                                <div class="mt-4">
                                    <p class="text-sm text-gray-400">Logo Saat Ini:</p>
                                    <img src="{{ asset('storage/' . $bankAccount->bank_logo) }}" alt="Bank Logo" class="mt-2 h-16 rounded-md">
                                </div>
                            @endif
                        </div>
