...
Route::get('/pending-assignment', function () {
    // Pastikan hanya pengguna terautentikasi yang bisa mengakses halaman ini.
    if (!auth()->check()) {
        return redirect('/login');
    }
    return view('pending');
})->middleware('auth')->name('pending.assignment');




Route::middleware('auth', 'unassigned.redirect')->group(function () {
        ...
        })->middleware(['auth', 'verified'])->name('dashboard');// ... sisa rute Anda tetap sama ...
